...
import code.name.monkey.pulsemusic.databinding.FragmentArtistDetailsBinding
...
abstract class AbsArtistDetailsFragment : AbsMainActivityFragment(R.layout.fragment_artist_details),
            ...
            if (bioContent != null && bioContent.trim { it <= ' ' }.isNotEmpty()) {
                binding.fragmentArtistContent.run {
                    ...
                    if (lastFmArtist.artist.stats.listeners.isNotEmpty()) {
                        ...
                        scrobblesLabel.show()
                        listeners.text =
                            PulseUtil.formatValue(lastFmArtist.artist.stats.listeners.toFloat())
                        scrobbles.text =
                            PulseUtil.formatValue(lastFmArtist.artist.stats.playcount.toFloat())
                    }
                }
            }
        }
    ...
    private fun loadArtistImage(artist: Artist) {
        Glide.with(requireContext()).asBitmapPalette().artistImageOptions(artist)
            .load(PulseGlideExtension.getArtistModel(artist)).dontAnimate()
