...
import code.name.monkey.pulsemusic.App.Companion.getContext
...
import code.name.monkey.pulsemusic.util.ArtistSignatureUtil
import code.name.monkey.pulsemusic.util.CustomArtistImageUtil.Companion.getFile
import code.name.monkey.pulsemusic.util.CustomArtistImageUtil.Companion.getInstance
import code.name.monkey.pulsemusic.util.MusicUtil.getMediaStoreAlbumCoverUri
...
// 🛠️ BURASI GÜNCELLENDİ: Glide 4.16.0 Nullability Fix
fun RequestBuilder<Drawable>.crossfadeListener(): RequestBuilder<Drawable> {
    return listener(object : RequestListener<Drawable> {
        override fun onLoadFailed(
            e: GlideException?,
            model: Any?,
            target: Target<Drawable>, // ? Kaldırıldı
        ...
        override fun onResourceReady(
            ...
            return if (isFirstResource) {
                ...
                val transition = factory.build(dataSource, isFirstResource)
                if (target is Transition.ViewAdapter) {
                    transition.transition(resource, target)
                }
                false
            }
        }
