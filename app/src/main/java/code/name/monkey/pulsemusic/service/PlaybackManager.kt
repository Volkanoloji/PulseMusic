...
class PlaybackManager(val context: Context) {
    ...
    fun maybeSwitchToCrossFade(crossFadeDuration: Int): Boolean {
        /* Switch to PulseExoPlayer if CrossFade duration is 0 and
                Playback is not an instance of PulseExoPlayer */
        if (playback !is PulseExoPlayer && crossFadeDuration == 0) {
            if (playback != null) {
                playback?.release()
            }
            playback = null
            playback = PulseExoPlayer(context)
            return true
    ...
    private fun createLocalPlayback(): Playback {
        // Set PulseExoPlayer when crossfade duration is 0 i.e. off
        return if (PreferenceUtil.crossFadeDuration == 0) {
            PulseExoPlayer(context)
        } else {
